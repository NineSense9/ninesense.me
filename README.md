...
```bash
sudo -u ninesense bash -c 'cd /opt/ninesense-guestbook/current; set -a; . /etc/ninesense/guestbook.env; set +a; ./venv/bin/python -m ninesense_guestbook.cli create-admin --username ninesense'
sudo -u ninesense bash -c 'cd /opt/ninesense-guestbook/current; set -a; . /etc/ninesense/guestbook.env; set +a; ./venv/bin/python -m ninesense_guestbook.cli reset-admin-password --username ninesense'
```

第一次使用密码登录后，后台会要求立即扫描二维码并启用动态两步验证。启用成功时只显示一次 10 个恢复码，应保存到密码管理器或其他独立安全位置。密码本身不会绕过动态验证直接创建会话。

完成密码和动态码验证后，同一浏览器默认保持登录 30 天。主动退出、在会话管理中撤销该设备或清除浏览器 Cookie 会立即结束登录；换浏览器或换设备仍需重新验证。

查看和撤销当前账户的全部后台会话：

```bash
sudo -u ninesense bash -c 'cd /opt/ninesense-guestbook/current; set -a; . /etc/ninesense/guestbook.env; set +a; ./venv/bin/python -m ninesense_guestbook.cli list-admin-sessions --username ninesense'
sudo -u ninesense bash -c 'cd /opt/ninesense-guestbook/current; set -a; . /etc/ninesense/guestbook.env; set +a; ./venv/bin/python -m ninesense_guestbook.cli revoke-admin-sessions --username ninesense'
```

如果验证设备和恢复码同时丢失，只能在服务器终端交互验证当前密码并重置两步验证。该操作会撤销全部会话、登录挑战和旧恢复码，下次登录必须重新绑定：

```bash
sudo -u ninesense bash -c 'cd /opt/ninesense-guestbook/current; set -a; . /etc/ninesense/guestbook.env; set +a; ./venv/bin/python -m ninesense_guestbook.cli reset-admin-mfa --username ninesense'
```

`NINESENSE_CONTACT_KEY` 与 `NINESENSE_SECURITY_KEY` 必须是两个不同的 32 字节随机密钥。前者保护访客联系方式，后者保护站长两步验证密钥，均不得进入 Git 仓库、日志或备份明文说明。
