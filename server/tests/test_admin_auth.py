...
def test_correct_login_sets_hardened_cookie_and_stores_only_hash(client, db_session):
...
def test_session_refresh_rotates_csrf_and_logout_revokes_session(client, db_session):
    create_admin(db_session)
...
def test_expired_session_is_rejected_and_removed(client, db_session):
    create_admin(db_session)
    login(client)
    stored = db_session.scalars(select(AdminSession)).one()
