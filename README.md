...
```bash
sudo -u ninesense bash -c 'set -a; . /etc/ninesense/guestbook.env; set +a; /opt/ninesense-guestbook/current/venv/bin/python -m ninesense_guestbook.cli list-admin-sessions --username ninesense'
sudo -u ninesense bash -c 'set -a; . /etc/ninesense/guestbook.env; set +a; /opt/ninesense-guestbook/current/venv/bin/python -m ninesense_guestbook.cli revoke-admin-sessions --username ninesense'
```

如果验证设备和恢复码同时丢失，只能在服务器终端交互验证当前密码并重置两步验证。该操作会撤销全部会话、登录挑战和旧恢复码，下次登录必须重新绑定：

```bash
sudo -u ninesense bash -c 'set -a; . /etc/ninesense/guestbook.env; set +a; /opt/ninesense-guestbook/current/venv/bin/python -m ninesense_guestbook.cli reset-admin-mfa --username ninesense'
```
